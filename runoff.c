...
void tabulate(void)
{
    for (int i =0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            //如果被投票的candidate沒有被runoff,在候選人的計票數裡加1
            if ( candidates[preferences[i][j]].eliminated == false)
            {
                candidates[preferences[i][j]].votes++;
                break;//執行這行程式碼會跳離這一層的for迴圈
            }
        }
    }
    return;
...
{
    int min_vote;
    //先找出第一個沒被runoff的candidate，將其票數暫存在變數min_vote裡，作為後續比較大小的基數
    for (int i = 0; i < candidate_count; i++)
    {
...
bool is_tie(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        //如果有某位candidate的votes不等於先前算出的最低票數，這局就一定不會是tie
